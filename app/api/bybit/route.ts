...
const BASE_URL = "https://api.bybit.com"; // один і той самий для demo та real
...
async function placeOrder(amountUSDT: number, entryPrice: number) {
  const stopLossPrice = (entryPrice * 0.98).toFixed(2);   // -2%
...
export async function GET() {
  try {
    ...
    const entryPrice = 65000; // для прикладу
    ...
    return NextResponse.json({
      ...
      takeProfit: (entryPrice * 1.05).toFixed(2),
      order,
      positions,
    });
  } catch (error) {
